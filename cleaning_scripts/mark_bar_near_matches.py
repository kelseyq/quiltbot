...
with open('../intermediate_data/bar-obit-items.json', 'r') as f:
    ...
    copyfile('../intermediate_data/bar-obit-items.json', '../cleaned_quiltdata/older_data/before_fuzzy_match_obits' +
         str(datetime.datetime.now()).split('.')[0] + ".json")
    with open('../intermediate_data/bar-obit-items.json', 'w') as f2:
        json.dump(obits, f2, indent=2, sort_keys=True)
        rename('../intermediate_data/bar-obit-items.json', '../intermediate_data/bar-obit-items.json')